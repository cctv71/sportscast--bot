...
# ---------------------------------------------------------------------------
# Global TV broadcaster lookup
# Uses TheSportsDB v2 API: /filter/tv/sport/{sport} — returns ALL upcoming
# TV events for a sport across every country globally.
# Also uses v1 eventstv.php?d={date} as a date-based fallback.
# Both approaches return (country, channel) pairs for every broadcaster.
# ---------------------------------------------------------------------------

# Maps bot sport keys to TheSportsDB sport name strings
SPORT_TV_NAME_MAP = {
    'football':         'Soccer',
    'cricket':          'Cricket',
    'rugby':            'Rugby',
...
_global_tv_cache = {}        # event_id -> [(country, channel), ...]
_sport_tv_cache  = {}        # sport_name -> (fetched_at, {event_id: [(country,channel)]})
_date_tv_cache   = {}        # date_str   -> (fetched_at, {event_id: [(country,channel)]})


# ---------------------------------------------------------------------------
# Hand-entered TV rights per league (league_id -> [(country, channel), ...]).
# TheSportsDB displays these on its league webpages but does NOT include them
# in any API response, so they are entered here from those pages directly.
# These are multi-year broadcast deals — update only when rights change hands.
# ---------------------------------------------------------------------------
LEAGUE_TV_TABLE = {
    # MotoGP (4407) — from TheSportsDB league page TV Rights section
    '4407': [
        ('UK', 'TNT Sports'), ('UK', 'itv'),
        ('USA', 'TruTV'),
        ('Australia', '10'), ('Australia', 'Fox Sports'),
        ('Canada', 'REV TV'), ('Canada', 'DAZN'),
        ('New Zealand', 'NOW'), ('New Zealand', 'Sky Sport NZ'),
        ('South Africa', 'SuperSport'),
        ('Hong Kong', 'Now Sports'),
        ('Korea', 'Spotv'), ('Korea', 'Coupang Play'),
        ('Singapore', 'Spotv'), ('Singapore', 'StarHub'),
        ('Germany', 'Sky'), ('Austria', 'Sky'), ('Austria', 'Servus TV'),
        ('France', 'Canal+'), ('Spain', 'DAZN'), ('Italy', '8tv'), ('Italy', 'Sky'),
        ('Netherlands', 'ZIGGO SPORT'), ('Belgium', 'rtbf auvio'), ('Belgium', 'PLaysports'),
        ('Portugal', 'Sport TV'), ('Brazil', 'Sport TV'), ('Argentina', 'ESPN+'),
        ('Mexico', 'ESPN+'), ('Japan', 'ntv'), ('Japan', 'hulu'),
        ('India', 'Eurosport'), ('China', 'Tencent Sports'),
        ('Poland', 'Polsat Sport'), ('Czech Rep', 'nova sport1'),
        ('Denmark', 'Viasatsport3'), ('Finland', 'V SPORT PLUS'),
        ('Norway', 'V SPORT 3'), ('Sweden', 'V SPORT MOTOR'),
        ('Turkey', 'smart SPORT'), ('Albania', 'Tring'),
    ],
    # Formula 1 (4370) — known multi-year deals
    '4370': [
        ('UK', 'Sky Sports F1'), ('UK', 'Channel 4'),
        ('USA', 'ESPN'), ('Canada', 'TSN'), ('Canada', 'DAZN'),
        ('Australia', 'Fox Sports'), ('Australia', 'Stan Sport'),
        ('New Zealand', 'Sky Sport NZ'),
        ('South Africa', 'SuperSport'),
        ('Hong Kong', 'Now Sports'), ('Korea', 'Coupang Play'),
        ('Singapore', 'StarHub'),
        ('Germany', 'Sky'), ('Austria', 'Servus TV'), ('Austria', 'ORF'),
        ('France', 'Canal+'), ('Spain', 'DAZN'), ('Italy', 'Sky'),
        ('Netherlands', 'Viaplay'), ('Brazil', 'Band'),
        ('Japan', 'DAZN'), ('India', 'FanCode'),
    ],
    # Formula E (4371)
    '4371': [
        ('UK', 'TNT Sports'), ('UK', 'Quest'),
        ('USA', 'Roku'), ('Australia', 'Stan Sport'),
        ('South Africa', 'SuperSport'), ('Hong Kong', 'Now Sports'),
    ],
    # World Rally Championship (4409)
    '4409': [
        ('UK', 'Rally.TV'), ('Worldwide', 'Rally.TV'),
        ('Australia', 'Stan Sport'), ('South Africa', 'SuperSport'),
    ],
    # NASCAR (4376)
    '4376': [
        ('USA', 'FOX'), ('USA', 'NBC'), ('USA', 'Prime Video'), ('USA', 'TNT'),
        ('UK', 'Viaplay'), ('Canada', 'TSN'),
    ],
    # IndyCar (4373)
    '4373': [
        ('USA', 'FOX'), ('UK', 'Sky Sports F1'), ('Canada', 'TSN'),
        ('Australia', 'Stan Sport'),
    ],
    # BTCC (4374)
    '4374': [('UK', 'itv4'), ('UK', 'itvX')],
    # British Superbikes (4375)
    '4375': [('UK', 'TNT Sports'), ('UK', 'Quest')],
    # Dakar Rally (4379)
    '4379': [
        ('UK', 'TNT Sports'), ('USA', 'Peacock'),
        ('France', 'France TV'), ('South Africa', 'SuperSport'),
    ],
    # PGA Tour (4425)
    '4425': [
        ('UK', 'Sky Sports Golf'), ('USA', 'CBS'), ('USA', 'NBC'), ('USA', 'Golf Channel'),
        ('Canada', 'TSN'), ('Australia', 'Fox Sports'), ('Australia', 'Kayo'),
        ('South Africa', 'SuperSport'), ('Hong Kong', 'Now Sports'),
        ('Korea', 'Coupang Play'), ('Singapore', 'StarHub'),
        ('New Zealand', 'Sky Sport NZ'),
    ],
    # DP World Tour (4426)
    '4426': [
        ('UK', 'Sky Sports Golf'), ('USA', 'Golf Channel'),
        ('South Africa', 'SuperSport'), ('Australia', 'Fox Sports'),
        ('New Zealand', 'Sky Sport NZ'),
    ],
    # LIV Golf (4753)
    '4753': [
        ('UK', 'itv'), ('USA', 'FOX'), ('USA', 'FS1'),
        ('Australia', 'Fox Sports'), ('Australia', 'Kayo'),
        ('Singapore', 'StarHub'),
    ],
    # LPGA Tour (4422)
    '4422': [
        ('UK', 'Sky Sports Golf'), ('USA', 'Golf Channel'),
        ('Korea', 'SBS Golf'), ('Australia', 'Fox Sports'),
    ],
    # PDC Darts (4554)
    '4554': [
        ('UK', 'Sky Sports'), ('UK', 'itv4'),
        ('Netherlands', 'Viaplay'), ('Germany', 'DAZN'), ('Germany', 'Sport1'),
        ('Australia', 'Fox Sports'), ('USA', 'DAZN'), ('Canada', 'DAZN'),
    ],
    # World Snooker Tour (4555)
    '4555': [
        ('UK', 'BBC'), ('UK', 'TNT Sports'), ('UK', 'Eurosport'),
        ('China', 'CCTV5'), ('Hong Kong', 'Now Sports'),
        ('Europe', 'Eurosport'), ('Europe', 'discovery+'),
    ],
    # ATP (4489) / WTA (4490) / Grand Slams
    '4489': [
        ('UK', 'Sky Sports Tennis'), ('USA', 'Tennis Channel'),
        ('Australia', 'beIN Sports'), ('Canada', 'TSN'),
        ('South Africa', 'SuperSport'), ('Hong Kong', 'Now Sports'),
    ],
    '4490': [
        ('UK', 'Sky Sports Tennis'), ('USA', 'Tennis Channel'),
    ...
    '4498': [  # Australian Open
        ('UK', 'Eurosport'), ('UK', 'discovery+'), ('Australia', '9Network'),
        ('USA', 'ESPN'), ('Canada', 'TSN'),
    ],
    '4499': [  # French Open
        ('UK', 'TNT Sports'), ('UK', 'Eurosport'), ('France', 'France TV'),
        ('USA', 'TNT'), ('Australia', '9Network'),
    ],
    '4500': [  # Wimbledon
        ('UK', 'BBC'), ('UK', 'TNT Sports'), ('USA', 'ESPN'),
        ('Australia', '9Network'), ('Canada', 'TSN'),
    ],
    '4501': [  # US Open
        ('UK', 'Sky Sports Tennis'), ('USA', 'ESPN'),
        ('Australia', '9Network'), ('Canada', 'TSN'),
    ],
    # Boxing (4445) / UFC (4443) / WWE (4449)
    '4445': [
        ('UK', 'DAZN'), ('UK', 'Sky Sports Boxing'), ('UK', 'TNT Sports'),
        ('USA', 'DAZN'), ('USA', 'ESPN+'), ('Canada', 'DAZN'),
        ('Australia', 'Fox Sports'), ('Australia', 'Stan Sport'),
    ],
    '4443': [
        ('UK', 'TNT Sports'), ('USA', 'ESPN+'), ('USA', 'Paramount+'),
        ('Australia', 'Kayo'), ('Canada', 'UFC Fight Pass'),
        ('South Africa', 'SuperSport'),
    ],
    '4449': [
        ('UK', 'Netflix'), ('USA', 'Netflix'), ('Worldwide', 'Netflix'),
    ],
}


def _parse_tv_rows(rows):
    """Parse a list of TV event rows into {event_id: [(country, channel), ...]}."""
    indexed = {}
    for row in rows:
        ev_id = str(row.get('idEvent') or '')
        if not ev_id:
            continue
        raw_channel = (row.get('strChannel') or row.get('strChannelName') or '').strip()
        country = (row.get('strCountry') or 'Unknown').strip()
        if not raw_channel:
            continue
        channels = [c.strip() for c in raw_channel.split('|') if c.strip()]
        indexed.setdefault(ev_id, [])
        for ch in channels:
            pair = (country, ch)
            if pair not in indexed[ev_id]:
                indexed[ev_id].append(pair)
    return indexed


def _get_sport_tv(sport_name):
    """v2 filter/tv/sport/{sport} — ALL global TV events for a sport (upcoming).
    Cached per sport for CACHE_EXPIRY_HOURS."""
    now = datetime.now()
    entry = _sport_tv_cache.get(sport_name)
    if entry:
        fetched_at, indexed = entry
        if now - fetched_at < timedelta(hours=CACHE_EXPIRY_HOURS):
            return indexed
    sport_slug = sport_name.lower().replace(' ', '_')
    data = _get(f'{V2_BASE}/filter/tv/sport/{sport_slug}', V2_HEADERS)
    rows = (data or {}).get('tvevent') or (data or {}).get('tvevents') or (data or {}).get('events') or []
    indexed = _parse_tv_rows(rows)
    _sport_tv_cache[sport_name] = (now, indexed)
    return indexed


def _get_date_tv(date_str):
    """v1 eventstv.php?d={date} — ALL global TV events on a specific date.
    Cached per date for CACHE_EXPIRY_HOURS."""
    now = datetime.now()
    entry = _date_tv_cache.get(date_str)
    if entry:
        fetched_at, indexed = entry
        if now - fetched_at < timedelta(hours=CACHE_EXPIRY_HOURS):
            return indexed
    data = _get(f'{V1_BASE}/eventstv.php?d={date_str}')
    rows = (data or {}).get('tvevent') or (data or {}).get('tvevents') or (data or {}).get('events') or []
    indexed = _parse_tv_rows(rows)
    _date_tv_cache[date_str] = (now, indexed)
    return indexed


def get_event_broadcasters(event, sport_key=''):
    """Global broadcaster data for an event from TheSportsDB.
    1. Check event-level cache
    2. Try v2 filter/tv/sport (all global channels for the sport)
    3. Try v1 eventstv.php?d={date} (all global channels for the date)
    4. Check strTVStation directly on the event
    Returns list of (country, channel) pairs. Empty if TheSportsDB has no data.
    """
    ev_id = str(event.get('idEvent') or '')

    # 1. Event-level cache
    if ev_id in _global_tv_cache:
        return _global_tv_cache[ev_id]

    tv_list = []

    # 2. v2 sport filter — best source for motorsport, golf, tennis etc.
    if sport_key and sport_key in SPORT_TV_NAME_MAP:
        sport_name = SPORT_TV_NAME_MAP[sport_key]
        sport_indexed = _get_sport_tv(sport_name)
        for pair in sport_indexed.get(ev_id, []):
            if pair not in tv_list:
                tv_list.append(pair)

    # 3. v1 date filter — good for football/rugby per-match TV data
    if not tv_list:
        date_str = event.get('dateEvent') or ''
        if date_str:
            date_indexed = _get_date_tv(date_str)
            for pair in date_indexed.get(ev_id, []):
                if pair not in tv_list:
                    tv_list.append(pair)

    # 4. Direct strTVStation field on the event
    if not tv_list:
        str_tv = (event.get('strTVStation') or '').strip()
        if str_tv:
            country = event.get('strCountry') or 'Unknown'
            for ch in str_tv.split('|'):
                ch = ch.strip()
                if ch:
                    tv_list.append((country, ch))

    # 5. League-level TV data from lookupleague (final fallback)
    # Checks BOTH strTVStation and strTvRights — the TV Rights field holds the
    # full global list shown on league pages (e.g. 'Albania - Tring\nUK - itv, TNT SPORTS')
    if not tv_list:
        league_id = event.get('idLeague') or ''
        if league_id:
            league = get_league_details(str(league_id))
            # a) strTVStation: simple comma/pipe-separated channel list
            str_tv_league = (league.get('strTVStation') or '').strip()
            if str_tv_league:
                league_country = league.get('strCountry') or 'Worldwide'
                for ch in str_tv_league.replace(',', '|').split('|'):
                    ch = ch.strip()
                    if ch:
                        tv_list.append((league_country, ch))
            # b) strTvRights: full global list, one 'Country - Channel' per line
            if not tv_list:
                raw_rights = (league.get('strTvRights') or league.get('strTVRights') or '').strip()
                if raw_rights:
                    raw_rights = raw_rights.replace('\r\n', '\n').replace('\r', '\n')
                    for line in raw_rights.split('\n'):
                        line = line.strip()
                        if not line:
                            continue
                        if ' - ' in line:
                            country, channels = line.split(' - ', 1)
                        elif '-' in line:
                            country, channels = line.split('-', 1)
                        else:
                            country, channels = 'Worldwide', line
                        country = country.strip()
                        # Channel part may hold several channels: 'itv, TNT SPORTS'
                        for ch in channels.replace('|', ',').split(','):
                            ch = ch.strip()
                            if ch:
                                pair = (country, ch)
                                if pair not in tv_list:
    ...
    # 6. Hand-entered TV rights table (league_id -> known broadcast deals).
    # TheSportsDB shows TV Rights on its league webpages but does NOT include
    # them in the API. These are the real multi-year deals from those pages.
    if not tv_list:
        league_id = str(event.get('idLeague') or '')
        for pair in LEAGUE_TV_TABLE.get(league_id, []):
            if pair not in tv_list:
                tv_list.append(pair)

    # Cache and return
    if ev_id:
        _global_tv_cache[ev_id] = tv_list
    return tv_list


# Backwards-compatible alias
def get_tv_with_fallback(event, sport_key=''):
    return get_event_broadcasters(event, sport_key)
        ...
        if sport_key in SEASON_FALLBACK_SPORTS:
            ...
            if not events:
                ...
                for try_season in [str(current_year), str(current_year - 1),
                    ...
                    if events:
                        break
    # Ensure every event carries its league id so broadcaster lookup always works
    for e in events:
        if not e.get('idLeague'):
